...
reallocBanksUntilLoop banks =
     unfoldr step (S.empty, banks) where
        step (history, banks)
            | S.member banks history = Nothing
            | otherwise = Just(banks, (S.insert banks history, banks')) where
...
main = do
    ...
    print $ (length . reallocBanksUntilLoop) banksVec